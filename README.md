...
out/make/squirrel.windows/x64/小红书运营台-0.5.0-Setup.exe
```
...
1. 关闭 `0.3.0`，覆盖安装当前 `0.4.1` 后启动应用；原数据库、加密凭证、任务和产物会继续保留。
2. 在“登录连接”点击“打开扩展目录”，在 Edge/Chrome 扩展管理页重新加载该稳定目录中的扩展 `0.2.1`。
3. 回到运营台点击“开始配对”，把 6 位配对码输入扩展弹窗。
4. 配对成功后在扩展弹窗点击“同步当前浏览器登录态”，切回运营台确认账号和最近同步时间更新。
...
1. 关闭 `0.4.0`，覆盖安装 `0.4.1`；SQLite、加密凭证、任务、产物和已有配对哈希不会被迁移或清除。
2. 在扩展管理页重新加载稳定目录中的扩展 `0.2.1`。
3. 安装版的“刷新状态”只读取后端保存的配对与同步时间，不再尝试检测另一个 Edge/Chrome 进程中的内容脚本。
4. 安装版请从扩展弹窗同步 Cookie；浏览器开发模式仍可使用页面内“检测助手”和“同步登录态”。
5. 错误配对码会显示剩余次数；解除扩展配对不会删除已加密 Cookie，左下角“小红书账号”仍代表已保存账号。

### 从 0.4.1 升级到 0.5.0

1. 关闭 `0.4.1`，覆盖安装 `0.5.0`；SQLite Schema 仍为 v3，原数据库、加密凭证、配对、任务和产物不会被迁移或清除。
2. 每个本地自然日首次成功启动会在 `%APPDATA%\小红书运营台\backups\daily` 创建一致性备份；同一天重复启动不会重复生成。
3. 打开“模型设置 → 备份与迁移”，可以立即备份、打开固定备份目录、预览恢复、导出脱敏迁移包或导入迁移包。
4. 本机完整备份保留 Windows 加密密文，适合同一 Windows 用户恢复；恢复前系统会再创建安全备份，然后关闭数据库并重启应用。
5. 脱敏迁移包会移除 Cookie、模型 Key、扩展配对和账号状态，并暂停未完成任务；导入另一台电脑后需要重新连接小红书并填写模型 Key。
6. 迁移包不包含可用凭证，但业务正文和 AI 产物未加密，只能通过公司可信渠道传输。

自动保留规则：最近 7 个日备份、最近 3 个升级/恢复前安全备份；手动备份不会由应用自动删除。损坏、被修改、Schema 过高或意外包含凭证的迁移包会在替换当前数据库前被拒绝。
...
7. 安装版点击扩展弹窗中的“同步当前浏览器登录态”；浏览器开发模式也可点击运营台中的“同步登录态”。配对会跨应用和浏览器重启保持，直到主动解除、扩展重装或新配对替换。
