...
## Windows 桌面试用版（D-004.1）

D-004.1 使用 Electron 包装现有 React 和 Express 应用，复用相同 HTTP API 和业务逻辑，并使用 Node 24 内置 `node:sqlite` 保存业务数据。安装版只监听 `127.0.0.1:8787`，不向局域网开放，也不要求业务员另行安装 Node.js、npm 或数据库。小红书 Cookie 和 AI 模型 Key 通过 Electron `safeStorage` 使用当前 Windows 用户的保护能力加密后写入 SQLite；浏览器扩展必须先完成一次性配对才能同步 Cookie。
...
out/make/squirrel.windows/x64/小红书运营台-0.4.1-Setup.exe
```
